{%- macro drop_pr_staging_schemas(PR_number) %}
    ...
    {% set pr_cleanup_query %}
        with pr_staging_schemas as (
            select schema_name
            from ANALYTICS.information_schema.schemata -- TODO: replace "demo" with your production database
            where
            schema_name like 'PR_NUM_'||{{ PR_number }}||'%'
        )

        select 
            'drop schema ANALYTICS.'||schema_name||';' as drop_command  -- TODO: replace "demo" with your production database
